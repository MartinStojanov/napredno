...
//todo: constructor
    ...
    public void printResults(){
        ...
        PatientService<T> findAllWithDoctor= (list, filter) ->
             list.stream()
                    .filter(i->i.getDoctor().equals(filter))
                    .collect(Collectors.toCollection(ArrayList::new));


        PatientService<T> countAllWithGenderAndAgeLessThanSixty = (list, filter) ->
                list.stream().filter(i -> i.getGender().equals(filter) & i.getAge()<=60)
                        .collect(Collectors.toCollection(ArrayList::new));


        PatientService<T> findAllWithNameSorted = (list,filter) ->
                list.stream().filter(i -> i.getName().equals(filter))
